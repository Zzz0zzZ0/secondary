...
每轮先扫描到期 Notes，再生成已到期消息，最后处理分类批次，避免分类积压阻挡现有消息。
消息生成支持 `HERMES_MESSAGE_WORKERS=1..4`（默认 1）；并发工作者逐条通过 SQLite 事务
领取线索，各自使用独立的任务目录，失败只重排对应线索。每轮仍最多处理 20 条，保留
CRM 重新核对、Notes 优先级、内容校验及人工审核。清理积压时可将分类批次调小，缩短
消息批次之间的等待；`HERMES_CLASSIFICATION_BACKLOG_DELAY_MINUTES` 控制分类批次的最短间隔。
...
Notes 对话读取同时支持邮件卡片和由 LinkedIn Context Importer 写入的
`createdByContext.source=crm.note.linkedin` 卡片。LinkedIn 卡片中的“我方/客户”消息
按原顺序展开，与邮件 Notes 共用最后回复侧判断、语义分析和人工审阅流程；回复渠道保持
为 LinkedIn，不会因为联系人同时存在邮箱而切换渠道。

普通 `conversation_follow_up` 也会读取当前 Notes 的完整会话顺序：顶层
`conversation_history` 传入生成模型，Review UI 同时展示同一份历史和逐字证据。沿用最新
业务消息的渠道及 Notes 联系方式，不因普通 CRM 快照缺少 LinkedIn 地址而误切到邮件。
Notes 内容指纹参与生成任务的幂等键，确保旧的无上下文结果不会被复用。

最后一条消息为我方发送时，新出现或内容变化的 Notes 会退回旧草稿；后续重复扫描同一
对话不会再次退回按期生成的跟进草稿。销售显示名变化不视为对话更新。

Notes 的 `no_action` 只表示当前不需要直接回复。已有我方消息且明确允许继续联系时，
完成的 Notes 分析会释放普通跟进调度：已有可靠分类则从释放时间起按分类周期设置
`next_action_at`（保留更晚的 CRM 跟进日期或 Notes `resume_at`）；尚未分类则先完成分类。
该转换幂等执行，重复扫描不会把日期不断后移。原分析结果保留，Notes 状态改为 `skipped`。
明确退订、禁止联系、权限不确定或仍有内部待办的记录不会因此放行。
...
- 只有 `createdAt` 回退时间：分类照常完成，并从本次分类时间起按分类间隔排期；
- 已有人工回复或自动跟进：继续按分类和跟进次数排期，到期生成新的普通跟进；
  `conversation_follow_up` 返回 `no_message` 且仍明确允许联系时，安排下一周期，不进入 `paused`。
...
消息任务的幂等键包含本次分类时间。即使 CRM 内容和跟进日期未变，重新分类后也会创建
新任务，避免复用已被替代的失败任务或上一轮的成功结果；同一次分类与排期重复入队仍复用同一任务。
...
- 主扫描与 Notes 均读取 CRM `recommendedById` 的正反向关系，输出 `crm_referral`。
- 分类验证和消息输入使用同一方向解析函数 `app/secondary/referrals.py`；模型文本证据
  仍需原文验证，结构化关联无需伪造备注引用。无关联时保留既有文本判断。
- 关联联系人不受当前队列生命周期过滤；排除自关联、已删除联系人，保留多名被推荐人。
- 推荐人有沟通历史时仍保留其角色；当前策略暂停其外联并转交被推荐人的独立队列。
- 历史修复备份与生成结果在 `outputs/referral-ui-repair/`。只更新本地状态和待审消息，
  不修改 CRM 推荐关系，不创建发送任务；已有人工编辑的正文予以保留。

### 推荐人不发信与被推荐人历史检查

`referral_handoff` 取代自动感谢推荐人。在线调度将推荐人待审消息退役并暂停其外联，
查验双向 CRM 关联后复用被推荐人的独立队列；转交本身不修改对方现有排期和待审引用。
被推荐人的发信历史在生成前实时读取，在批准前用 `referral_history_check.source_hash`
再校验一次。CRM 读取失败不会视为无历史。Notes 来信优先，日期缺失人工核对；已发信
则按实际发信日期排期并进入提前审阅窗口，只有没有任何沟通或发送证据时才允许首次联系。

## 提前生成窗口与独立审阅标记

- 跟进间隔按现有确定性算法计算；间隔 >30 天时提前 7 天准备，否则提前 3 天。
- `_claim_due_action` 和 `_next_wakeup_at` 使用同一窗口计算，并考虑 `generation_retry_at`。
  等待审阅/投递的联系人不能再次被领取。失败重试不更改 `next_action_at`；已成功发出才增加跟进计数。
- 新草稿携带 `review_schedule`：计划跟进时间、准备时间、提前天数、CRM 业务哈希、Notes 历史哈希。
  模型的 `generation_ready` 与 `delivery_due` 分开表达；不会把未来时间伪装成已到期。
- `no_message` 如果允许下次跟进，则保留下一次日期，并在该日期再检查，避免短周期提前窗口引发连续生成。
- `POST /api/review/messages/{id}/reviewed` 使用审核人及 `expected_updated_at` 乐观版本检查；
  标记存于 `crm_snapshot.content_review`，消息仍为 `pending_review`，不写 `delivery_outbox`。
- 保存或重新生成清除审阅标记；重生成在本地保存 `previous-draft.json`，并在发布前检查消息版本及最新上下文。
- 未到跟进时间禁止批准；到期后沿用人工批准机制。批准时和领取投递时都检查 CRM/Notes，CRM 不可用时不放行。
  领取时若确认上下文变化，旧投递取消，消息版本递增并退回待审，以便重审后使用新的幂等键。
- 蓝/黄/橙/红表示提前审阅、24 小时内、已到期、逾期至少 48 小时；客户来信沿用绿色，未排期沿用灰色。
  时间标签和按钮每 30 秒原位更新，不刷新页面或替换编辑器。

## 按 CRM source 指定渠道

- 来源包含 `agent`（忽略大小写）：LinkedIn。来源 `CRMGEN_JIN` / CRM跟进：email。
- `app.secondary.channels` 统一普通跟进、Notes 草稿和重生成的输出选择；CRM 导出同时给出对应渠道。
- 指定渠道没有有效地址时进入 `needs_contact`，不借用另一个渠道；旧稿渠道不符时禁止批准和投递。
- 原始 Notes 的渠道、正文和日期保持不变；新发送渠道可以与历史渠道不同，所有真实发信仍参与排期去重。
- 来源是路由元数据，单纯新增该字段不会重开 Notes 内部待办。
- 验证：`.venv/bin/python -m unittest discover -s tests`；本次迁移和验收记录位于 `outputs/agent-source-channel-20260908/`。
